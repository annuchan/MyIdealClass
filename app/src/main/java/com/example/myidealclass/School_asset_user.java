...
public class School_asset_user extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        recyclerView3 = findViewById(R.id.recyclerViewImportant_information);
        recyclerView3.setLayoutManager(new LinearLayoutManager(this));

        apiService = RetrofitClient.getApiService();

        loadImportant_information();
    }

    private void loadImportant_information() {
        ...
        call.enqueue(new Callback<List<School_asset>>() {
            ...
            public void onResponse(Call<List<School_asset>> call, Response<List<School_asset>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    List<School_asset> school_asset = response.body();
                    Log.d("API_SUCCESS", "Получено " + school_asset.size() + " events.");
                    adapter = new School_assetAdapter(School_asset_user.this, school_asset); // Используем EventsAdapter
